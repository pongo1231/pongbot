...
void ConVarHolder::Init()
{
	if (!_ConVarHolder)
	{
		...
		_ConVarHolder->CVarBotNodeTouchRadius = new ConVar("pongbot_bot_goto_nodetouchradius", "50.0", 0, "Min distance to node to count as 'touched'");
		...
		_ConVarHolder->CVarBotPrimaryWeaponDist = new ConVar("pongbot_bot_primwepdist", "800.0", 0, "Min distance where bots use their primary weapon");
		_ConVarHolder->CVarBotSecondaryWeaponDist = new ConVar("pongbot_bot_secwepdist", "300.0", 0, "Min distance where bots use their secondary weapon");
		_ConVarHolder->CVarBotBehaviourUpdateTick = new ConVar("pongbot_bot_brain_updatetick", "0.2", 0, "How often the bots will check for updated tasks");
...
void ConVarHolder::Destroy()
{
	if (_ConVarHolder)
	{
		...
		delete _ConVarHolder->CVarBotPrimaryWeaponDist;
		delete _ConVarHolder->CVarBotSecondaryWeaponDist;
		delete _ConVarHolder->CVarBotBehaviourUpdateTick;
